...
# Chemins des médias (dans ton dossier local)
...
logo2 = ImageClip(logo_fusikab).resize(height=900).set_duration(duration_each).set_position("center").fadein(1).fadeout(1)
...
# Pool logo en fin
pool = ImageClip(pool_logo).resize(height=200).set_duration(4).fadein(1).fadeout(1).set_position("center")\
    .on_color(size=video_size, color=(0, 0, 0))
